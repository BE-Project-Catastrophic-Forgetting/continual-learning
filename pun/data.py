...
def get_dataset(name, type='train', download=True, capacity=None, permutation=None, dir='./datasets',
    ...
    data_name = 'mnist' if name=='mnist28' else name  # if name is mnist28 change it to mnist
    dataset_class = AVAILABLE_DATASETS[data_name]  # python class for dataset under torchvision

    # specify image-transformations to be applied
    dataset_transform = transforms.Compose([  #  brings all the tranforms together
        *AVAILABLE_TRANSFORMS[name],
        transforms.Lambda(lambda x, p=permutation: _permutate_image_pixels(x, p)), #  Apply a user-defined lambda as a transform.
    ])

    # load data-set
    dataset = dataset_class('{dir}/{name}'.format(dir=dir, name=data_name), train=False if type=='test' else True,
                            download=download, transform=dataset_transform, target_transform=target_transform)  # <-- datasets.mnist.__init__()

    # print information about dataset on the screen
    if verbose:
        print(" --> {}: '{}'-dataset consisting of {} samples".format(name, type, len(dataset)))  # --> mnist: 'train' -dataset consisting of {n} samples
...
AVAILABLE_TRANSFORMS = {
    'mnist': [
        transforms.Pad(2),  #  Pad the given image on all sides with the given “pad” value.
    ...
    if name == 'permMNIST':
        ...
        if not only_config: # not passed in main.py, taking default value
            ...
            train_dataset = get_dataset('mnist', type="train", permutation=None, dir=data_dir,
                                        target_transform=None, verbose=verbose)                 # not doing any permutations while getting the original dataset
            test_dataset = get_dataset('mnist', type="test", permutation=None, dir=data_dir,
                                       target_transform=None, verbose=verbose)
            # generate permutations
            if exception:
                permutations = [None] + [np.random.permutation(config['size']**2) for _ in range(tasks-1)] # permutating the images for every task
            else:
            ...
            for task_id, perm in enumerate(permutations):
                ...
                ) if scenario in ('task', 'class') else None  #  creating lambda function for permutations
